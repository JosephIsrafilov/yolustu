...
def main():
    ...
    market_rates_file = os.path.join(os.path.dirname(script_dir), "app", "domains", "ai", "market_rates.json")
    
    if not os.path.exists(input_file):
        print(f"Please create a file named 'raw_fb_posts.txt' in {script_dir}")
        print("Paste the text you copied from Facebook into that file, then run this script again.")
    ...
    if not raw_text or raw_text == "Paste Facebook posts here...":
        print("The file 'raw_fb_posts.txt' is empty or contains the placeholder text. Please paste real data.")
        sys.exit(1)
