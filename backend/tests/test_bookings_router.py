...
def test_bookings_create_endpoint(monkeypatch):
    monkeypatch.setattr(
        BookingsService,
        "create_booking",
        lambda self, booking_in, current_user: sample_booking_response("pending"),
    )
...
def test_bookings_my_and_requests_endpoints(monkeypatch):
    monkeypatch.setattr(
        BookingsService,
        "get_my_bookings",
        lambda self, current_user: [sample_booking_response("accepted")],
    )
    monkeypatch.setattr(
        BookingsService,
        "get_booking_requests",
        lambda self, current_user: [sample_booking_response("pending")],
    )
    token = get_access_token()
    ...
    requests_response = client.get(
        "/api/v1/bookings/requests", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_bookings_confirm_reject_cancel_endpoints(monkeypatch):
    monkeypatch.setattr(
        BookingsService,
        "confirm_booking",
        lambda self, booking_id, current_user: sample_booking_response("accepted"),
    )
    monkeypatch.setattr(
        BookingsService,
        "reject_booking",
        lambda self, booking_id, current_user: sample_booking_response("rejected"),
    )
    monkeypatch.setattr(
        BookingsService,
        "cancel_booking",
        lambda self, booking_id, current_user: sample_booking_response("cancelled"),
    )
    token = get_access_token()
