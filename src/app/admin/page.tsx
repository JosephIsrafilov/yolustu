...
import { Users, Map, CalendarCheck, Clock, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Ümumi rezervlər', value: bookings.length, icon: CalendarCheck, color: 'text-accent-500' },
    { label: 'Gözləyən', value: bookings.filter((b) => b.status === 'pending').length, icon: Clock, color: 'text-warn-500' },
    { label: 'Tamamlanmış', value: trips.filter((t) => t.status === 'completed').length, icon: CheckCircle, color: 'text-brand-600' },
    { label: 'Bloklanmış', value: users.filter((u) => u.isBlocked).length, icon: AlertTriangle, color: 'text-danger-500' },
  ];
