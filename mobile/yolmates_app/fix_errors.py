...
text = re.sub(
    r"class _SummaryCard extends StatelessWidget \{[\s\S]*?Widget build\(BuildContext context, WidgetRef ref\) \{[\s\S]*?final l10n = ref.watch\(l10nProvider\);",
    "class _SummaryCard extends StatelessWidget {\n  final double average;\n  final int count;\n\n  const _SummaryCard({required this.average, required this.count});\n\n  @override\n  Widget build(BuildContext context) {",
    text,
)

text = re.sub(
    r"class _ReviewCard extends StatelessWidget \{[\s\S]*?Widget build\(BuildContext context, WidgetRef ref\) \{[\s\S]*?final l10n = ref.watch\(l10nProvider\);",
