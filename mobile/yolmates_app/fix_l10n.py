...
text = text.replace("    final l10n = ref.watch(l10nProvider);\n    final isPending = status == VerificationStatus.pending;", "    final isPending = status == VerificationStatus.pending;")
...
text = text.replace("class ReviewsScreen extends StatelessWidget {", "class ReviewsScreen extends ConsumerWidget {")
...
text = text.replace("class _TripCard extends StatelessWidget {", "class _TripCard extends ConsumerWidget {")
text = re.sub(r"Widget build\(BuildContext context\) \{", "Widget build(BuildContext context, WidgetRef ref) {\n    final l10n = ref.watch(l10nProvider);", text)
