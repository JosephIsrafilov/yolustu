...
l10n_file = (
    r"E:\yolustu\mobile\yolmates_app\lib\core\localization\app_localizations.dart"
)
...
dv_replacements = [
    ...
    (
        "Text('$docType tTsdiq ? ?n g ndTrildi')",
        "Text('$docType ${ref.read(l10nProvider).verificationSent}')",
    ),
    ("Text('XTta ba? verdi: $e')", "Text('${ref.read(l10nProvider).commonError}: $e')"),
    (
        "Text('S?r?c? statusu tTsdiqlTndi (MOCK)')",
        "Text(ref.read(l10nProvider).verificationMockApproved)",
    ),
    ...
    (
        "Text(isApproved ? 'TTsdiq' : 'Y?klT')",
        "Text(isApproved ? l10n.verificationApproveBtn : l10n.verificationUploadBtn)",
    ),
    ...
    (
        "'TTsdiq prosesi bir ne T dTqiqT  TkT bilTr. Status yenilTndikdT S?r?c? Paneli aktivlT?TcTk.'",
        "l10n.verificationNote",
    ),
]
...
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationIdCard", "l10n.verificationIdCard"
)
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationSelfie", "l10n.verificationSelfie"
)
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationLicense", "l10n.verificationLicense"
)
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationIdCardDesc", "l10n.verificationIdCardDesc"
)
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationSelfieDesc", "l10n.verificationSelfieDesc"
)
dv_text = dv_text.replace(
    "ref.read(l10nProvider).verificationLicenseDesc", "l10n.verificationLicenseDesc"
)
...
if "class _Note extends StatelessWidget" in dv_text:
    dv_text = dv_text.replace(
        "class _Note extends StatelessWidget", "class _Note extends ConsumerWidget"
    )
    dv_text = dv_text.replace(
        "Widget build(BuildContext context) {",
        "Widget build(BuildContext context, WidgetRef ref) {",
        1,
    )  # wait, replace only inside _Note!
    dv_text = re.sub(
        r"class _Note extends ConsumerWidget \{[\s\S]*?Widget build\(BuildContext context\) \{",
        "class _Note extends ConsumerWidget {\n  const _Note();\n\n  @override\n  Widget build(BuildContext context, WidgetRef ref) {\n    final l10n = ref.watch(l10nProvider);",
        dv_text,
    )
...
# profile screen
profile_path = (
    r"E:\yolustu\mobile\yolmates_app\lib\features\profile\profile_screen.dart"
)
replace_in_file(
    profile_path,
    [
        ("const Text('İmtina')", "Text(l10n.commonDismiss)"),
        ("const Text('Imtina')", "Text(l10n.commonDismiss)"),
    ],
)

# reviews screen
reviews_path = (
    r"E:\yolustu\mobile\yolmates_app\lib\features\reviews\reviews_screen.dart"
)
...
# trip_list_screen
trip_list_path = (
    r"E:\yolustu\mobile\yolmates_app\lib\features\trips\trip_list_screen.dart"
)
...
tl_text = tl_text.replace(
    "Text('${trip.availableSeats} bos yer'",
    "Text('${trip.availableSeats} ${l10n.commonAvailableSeats}'",
)
