...
def test_yolmates_flows():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            chat_buttons = page.locator('button[aria-label="Chat"], button[aria-label="Söhbət"]')
        ...
        try:
            ...
            if error_message.count() > 0 and error_message.is_visible():
                log_err("Chat list failed to load: 'Sohbetleri yuklemek olmadi' error is present.")
            else:
                page.wait_for_selector(".chat-list-item, .empty-state", timeout=10000)
                log_ok("Chat list loaded successfully without errors.")
        except Exception as e:
            log_warn("Timeout while verifying chats page. Ensure backend API is responding.")
        ...
        try:
            ...
            page.click('button[type="submit"]')
            log_ok("Clicked Send Code on Forgot Password page. This triggers the SMTP OTP logic on the backend.")
            ...
            log_ok("Navigated to OTP verification step. SMTP Email should have been sent successfully.")
